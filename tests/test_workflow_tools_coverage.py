...
class TestGetAuthAndConfig(IsolatedAsyncioTestCase):
    ...
    async def test_swapped_order(self):
        auth_manager, server_config = _make_auth_and_config()
        am, sc = _get_auth_and_config(server_config, auth_manager)
        self.assertIs(am, auth_manager)
        self.assertIs(sc, server_config)

    async def test_duck_typed_get_headers_on_first_arg(self):
    ...
    async def test_no_get_headers_raises(self):
    ...
    async def test_no_instance_url_raises(self):
...
class TestGetWorkflowDetailsExtra(IsolatedAsyncioTestCase):
    ...
    async def test_missing_workflow_id_returns_error(self):
        ...
        result = await get_workflow_details(self.auth_manager, self.server_config, {})
        self.assertIn("error", result)
    ...
    async def test_generic_exception_returns_error(self, mock_get):
        ...
        result = await get_workflow_details(
            self.auth_manager, self.server_config, {"workflow_id": "wf1"}
        )
        self.assertIn("error", result)

    async def test_bad_auth_config_returns_error(self):
        """Lines 271-273: ValueError from _get_auth_and_config."""
        result = await get_workflow_details(MagicMock(spec=[]), MagicMock(spec=[]), {"workflow_id": "x"})
        self.assertIn("error", result)
...
class TestListWorkflowVersionsExtra(IsolatedAsyncioTestCase):
    ...
    async def test_missing_workflow_id_returns_error(self):
        ...
        result = await list_workflow_versions(self.auth_manager, self.server_config, {})
        self.assertIn("error", result)
    ...
    async def test_bad_auth_config_returns_error(self):
        """Lines 321-323."""
        result = await list_workflow_versions(MagicMock(spec=[]), MagicMock(spec=[]), {"workflow_id": "x"})
        self.assertIn("error", result)
...
class TestGetWorkflowActivitiesExtra(IsolatedAsyncioTestCase):
    ...
    async def test_bad_auth_config_returns_error(self):
        """Lines 381-383."""
        result = await get_workflow_activities(MagicMock(spec=[]), MagicMock(spec=[]), {"workflow_id": "x"})
        self.assertIn("error", result)

    @patch.object(httpx.AsyncClient, "get", new_callable=AsyncMock)
    async def test_no_published_versions_returns_error(self, mock_get):
        """Line 409: no published versions found."""
        mock_get.return_value = _ok_response({"result": []})
        result = await get_workflow_activities(
            self.auth_manager, self.server_config, {"workflow_id": "wf1"}
        )
        self.assertIn("error", result)

    @patch.object(httpx.AsyncClient, "get", new_callable=AsyncMock)
    async def test_request_exception_during_version_fetch(self, mock_get):
        """Lines 415-417: RequestException during version fetch."""
        mock_get.side_effect = httpx.HTTPError("net err")
        result = await get_workflow_activities(
            self.auth_manager, self.server_config, {"workflow_id": "wf1"}
        )
        self.assertIn("error", result)
    ...
    @patch.object(httpx.AsyncClient, "get", new_callable=AsyncMock)
    async def test_request_exception_during_activities_fetch(self, mock_get):
        ...
        def side_effect(*args, **kwargs):
            ...
            raise httpx.HTTPError("activities err")
...
class TestCreateWorkflowExtra(IsolatedAsyncioTestCase):
    ...
    async def test_missing_name_returns_error(self):
        ...
        result = await create_workflow(self.auth_manager, self.server_config, {})
        self.assertIn("error", result)

    async def test_bad_auth_config_returns_error(self):
        """Lines 471-473."""
        result = await create_workflow(MagicMock(spec=[]), MagicMock(spec=[]), {"name": "x"})
        self.assertIn("error", result)

    @patch.object(httpx.AsyncClient, "post", new_callable=AsyncMock)
    async def test_generic_exception_returns_error(self, mock_post):
        """Lines 513-515."""
...
class TestUpdateWorkflowExtra(IsolatedAsyncioTestCase):
    ...
    async def test_missing_workflow_id_returns_error(self):
        """Line 546."""
        result = await update_workflow(self.auth_manager, self.server_config, {})
        self.assertIn("error", result)
    ...
    async def test_table_and_active_and_attributes(self, mock_patch):
    ...
    @patch.object(httpx.AsyncClient, "patch", new_callable=AsyncMock)
    async def test_generic_exception_returns_error(self, mock_patch):
        """Lines 586-588."""
...
class TestActivateDeactivateExtra(IsolatedAsyncioTestCase):

    def setUp(self):
        self.auth_manager, self.server_config = _make_auth_and_config()

    async def test_activate_missing_workflow_id(self):
        """Line 619."""
        result = await activate_workflow(self.auth_manager, self.server_config, {})
        self.assertIn("error", result)

    async def test_activate_bad_auth_config(self):
        """Lines 613-615."""
        result = await activate_workflow(MagicMock(spec=[]), MagicMock(spec=[]), {"workflow_id": "x"})
        self.assertIn("error", result)

    @patch.object(httpx.AsyncClient, "patch", new_callable=AsyncMock)
    async def test_activate_generic_exception(self, mock_patch):
        """Lines 642-644."""
        mock_patch.side_effect = Exception("boom")
    ...
    async def test_deactivate_missing_workflow_id(self):
        """Line 675."""
        result = await deactivate_workflow(self.auth_manager, self.server_config, {})
        self.assertIn("error", result)

    async def test_deactivate_bad_auth_config(self):
        """Lines 669-671."""
        result = await deactivate_workflow(MagicMock(spec=[]), MagicMock(spec=[]), {"workflow_id": "x"})
        self.assertIn("error", result)

    @patch.object(httpx.AsyncClient, "patch", new_callable=AsyncMock)
    async def test_deactivate_generic_exception(self, mock_patch):
        """Lines 698-700."""
        mock_patch.side_effect = Exception("boom")
...
class TestAddWorkflowActivityExtra(IsolatedAsyncioTestCase):
    ...
    async def test_missing_activity_name(self):
...
class TestUpdateWorkflowActivityExtra(IsolatedAsyncioTestCase):
    ...
    async def test_bad_auth_config_returns_error(self):
        """Lines 797-799."""
        result = await update_workflow_activity(
            MagicMock(spec=[]), MagicMock(spec=[]),
            {"activity_id": "a1", "name": "n"},
        )
        self.assertIn("error", result)

    async def test_missing_activity_id(self):
        """Line 816: missing activity_id."""
        result = await update_workflow_activity(self.auth_manager, self.server_config, {})
        self.assertIn("error", result)
...
class TestDeleteWorkflowActivityExtra(IsolatedAsyncioTestCase):
    ...
    @patch.object(httpx.AsyncClient, "delete", new_callable=AsyncMock)
    async def test_request_exception_returns_error(self, mock_delete):
        """Lines 884-886."""
        mock_delete.side_effect = httpx.HTTPError("net")
        result = await delete_workflow_activity(
            self.auth_manager, self.server_config, {"activity_id": "a1"}
        )
        self.assertIn("error", result)
...
class TestReorderWorkflowActivitiesExtra(IsolatedAsyncioTestCase):
    ...
    async def test_bad_auth_config_returns_error(self):
        """Lines 914-916."""
        result = await reorder_workflow_activities(
            MagicMock(spec=[]), MagicMock(spec=[]),
    ...
    async def test_missing_activity_ids_returns_error(self):
        """Line 924."""
        result = await reorder_workflow_activities(
            self.auth_manager, self.server_config, {"workflow_id": "wf1"}
        )
    ...
    @patch.object(httpx.AsyncClient, "patch", new_callable=AsyncMock)
    async def test_per_item_request_exception_recorded(self, mock_patch):
        """Lines 947-953: per-item RequestException is recorded in results."""
        mock_patch.side_effect = httpx.HTTPError("item err")
        result = await reorder_workflow_activities(
            self.auth_manager, self.server_config,
    ...
    @patch.object(httpx.AsyncClient, "patch", new_callable=AsyncMock)
    async def test_generic_exception_in_outer_try(self, mock_patch):
...
class TestDeleteWorkflow(IsolatedAsyncioTestCase):
    ...
    async def test_missing_workflow_id_returns_error(self):
        """Line 993."""
        result = await delete_workflow(self.auth_manager, self.server_config, {})
        self.assertIn("error", result)
    ...
    @patch.object(httpx.AsyncClient, "delete", new_callable=AsyncMock)
    async def test_request_exception(self, mock_delete):
        """Lines 1007-1009."""
        mock_delete.side_effect = httpx.HTTPError("net err")
        result = await delete_workflow(
            self.auth_manager, self.server_config, {"workflow_id": "wf1"}
        )
        self.assertIn("error", result)
    ...
    @patch.object(httpx.AsyncClient, "delete", new_callable=AsyncMock)
    async def test_generic_exception(self, mock_delete):
        """Lines 1010-1012."""
        mock_delete.side_effect = Exception("boom")
        result = await delete_workflow(
            self.auth_manager, self.server_config, {"workflow_id": "wf1"}
